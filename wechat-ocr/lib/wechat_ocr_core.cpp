...
//
// 算法:
//   1) ocr_capture() 对微信窗口做 OCR，找出所有 HH:MM 格式文本
//   2) 取第一个时间戳的右边缘 + 25px = 列边界
//      如果窗口宽度 < 2000，再减 10px
//   3) 找不到时间戳则返回错误
//
// 输出:
//   - win.x = 面板绝对x + 边界
//   - win.y = 面板绝对y + 35（标题栏高度）
//   - win.w = 面板宽度 - 边界（第三列宽度）
//   - win.h = 面板高度 - 220（聊天区域高度）
//   - boxes = 第三列内的文字框（坐标相对第三列左上角）
// ============================================================
