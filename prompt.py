...
2. 项目目录结构和函数信息已提供，除非有明确需求，否则无需重复确认

3. !!特别重要如果用户需求简单，一个简单 py 脚本就能实现和验证或者一段 python 代码就能实现和验证，
   可以直接回复实现方案，不需要再多次交互。如果需要创建文件(假设为demo.py) 
   同时需要多返回一个#shell mv demo.py demo.py_bak #end 标签,避免覆盖已有文件。

3. 规划实现方案,把实现过程拆分成任务进度清单，并自己维护任务清单的工作状态,
   用#task #end 包裹作为返回客户端内容的一部分,标签包裹的内容一行一个任务还有任务状态，本地会原样返回给你。

4. 返回带#标签指令 #end 的内容，让本地解析并执行相应的操作,返回的内 #end容分两部分一部分是操作本地的指令标签内容，
   第二部分是人类可以理解的自然语言，作为你对实现需求的必要描述,文字简练通熟易懂。
   如有必要同一个指令标签可以多次出现，本地会多次匹配并执行。

5  #history #end 标签是每次返回都必须有的，记录了上下文信息，每次收到请求都要把当前收到的内容总结成简短的几句话，
   添加到 #history #end 标签里。如果收到的 #history #end 没有内容，说明是第一次请求。

6. 根据客户端返回的内容检查执行结果，根据需要进行下一步或者确认是否完成目标。

7. 完成任务后直接返回结果 [success!]
##
