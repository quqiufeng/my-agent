...
3. 返回带#标签指令 #end 的内容，让本地解析并执行相应的操作,返回的内 #end容分两部分一部分是操作本地的指令标签内容，
...
- #history #end 标签是必须的，记录了上下文信息，每次收到请求都要把当前收到的内容总结成简短的几句话，新增到已经有的 #history #end 标签里。如果收到的 #history #end 没有包裹内容，只是说明当前是这次会话的第一次请求。
- 与客户端如果有多次交互，需要自己维护上下文信息，返回内容中要包含 #history #end 包裹的内容，维护最近30条即可，控制好记录总数
- 如果执行失败，分析错误原因并修复"""
